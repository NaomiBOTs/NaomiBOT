...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Se foi mais um viadinho kkkkkk🐊 @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '🐊 Aguarde um pouco ae viadinho... 🐊',
			...
			switch(command) {
				...
				case 'add':
					...
					if (args.length < 1) return reply('Voce quer adicionar um gênio ou sua mãe? Coloca o número do cara ae man?')
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pedido Recebido, Segredo : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('1para ativar, 0  para desativar')
					}
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
